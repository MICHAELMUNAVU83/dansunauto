defmodule Dansunauto.Chat do
  ...
  alias Dansunauto.Chat.{Session, Message}
  ...
  def subscribe_visitor(visitor_id),
    do: Phoenix.PubSub.subscribe(Dansunauto.PubSub, topic_visitor(visitor_id))

  def unsubscribe_visitor(visitor_id),
    do: Phoenix.PubSub.unsubscribe(Dansunauto.PubSub, topic_visitor(visitor_id))

  def subscribe_admin(),
    do: Phoenix.PubSub.subscribe(Dansunauto.PubSub, topic_admin())

  def subscribe_admin_session(session_id),
    do: Phoenix.PubSub.subscribe(Dansunauto.PubSub, topic_admin_session(session_id))

  defp broadcast_visitor(visitor_id, event, payload),
    do: Phoenix.PubSub.broadcast(Dansunauto.PubSub, topic_visitor(visitor_id), {event, payload})
    ...
    case result do
      {:ok, message} ->
        ...
        if message.sender == "visitor" do
          ...
          if is_first do
            Task.start(fn ->
              Dansunauto.ChatNotifier.notify_admin_new_chat(session, message)
            end)
          end
        else
  ...
  def search_products(query) when is_binary(query) and byte_size(query) > 0 do
    ...
    from(p in Dansunauto.Products.Product,
