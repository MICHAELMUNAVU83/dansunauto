defmodule Dansunauto.OrderNotifier do
  alias Dansunauto.Gmail
  import DansunautoWeb.Format, only: [price: 1]
  ...
  def send_confirmation(
    ...
    base = Dansunauto.AppConfig.site_url()
    track_url = "#{base}/success?reference=#{reference}"

    whatsapp_url =
      "https://wa.me/254796770862?text=Hi%20Dansunauto%2C%20my%20order%20ref%20is%20#{reference}"
    ...
    body =
      Gmail.branded_email(
        ...
        """,
        preview:
          "Your order #{reference} is confirmed — thanks for shopping with Dansun Auto Care!",
        header_label: "Order Confirmation"
      )

    Gmail.send_email(email, "Order Confirmed — #{reference} 🧾", body)
  end
  ...
  def send_admin_notification(
    ...
    admin_email = Dansunauto.AppConfig.admin_email()
    admin_url = "#{Dansunauto.AppConfig.site_url()}/admin/orders"
    items_html = build_items_html(items)
    ...
    Gmail.send_email(admin_email, "🧾 New Order — #{reference} (KES #{price(total)})", body)
  end
