defmodule DansunautoWeb.PromotionLive.Index do
  use DansunautoWeb, :admin_live_view

  alias Dansunauto.Promotions
  alias Dansunauto.Promotions.PromoCode
  ...
  def handle_event("edit_promo", %{"id" => id}, socket) do
    ...
    form = %{
      ...
      "expires_at" =>
        if(promo.expires_at, do: Calendar.strftime(promo.expires_at, "%Y-%m-%dT%H:%M"), else: "")
    }
  ...
  defp fmt(n), do: DansunautoWeb.Format.price(n)
  ...
  def render(assigns) do
      ...
      <div class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-brand to-[#8b0014] px-7 py-6 text-white shadow-md">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 class="mt-0.5 font-display uppercase tracking-wide text-2xl font-bold">
              Promo Codes
            </h1>
            <p class="mt-1 text-xs text-red-200">
              {length(@promo_codes)} codes · track influencer-driven sales
            </p>
          </div>
          <button
            phx-click="new_promo"
    ...
    <!-- ── Create / Edit Form ── -->
      <%= if @show_form do %>
        <div class="overflow-hidden rounded-3xl border border-brand/20 bg-white shadow-sm">
          ...
          <form
            phx-submit="save_promo"
            phx-change="update_form"
            class="grid gap-5 px-6 py-5 sm:grid-cols-2 lg:grid-cols-3"
          >
            <!-- Code -->
            <div>
              ...
              <input
                ...
                value={@form["code"]}
                placeholder="GRACE20"
                class="w-full rounded-xl border border-line px-3.5 py-2.5 font-mono text-sm uppercase tracking-widest placeholder-gray-300 transition focus:border-brand/50 focus:outline-none focus:ring-2 focus:ring-brand/10"
              />
    ...
    <!-- Influencer name -->
            <div>
              ...
              <input
                ...
                class="w-full rounded-xl border border-line px-3.5 py-2.5 text-sm placeholder-gray-300 transition focus:border-brand/50 focus:outline-none focus:ring-2 focus:ring-brand/10"
              />
            </div>
            ...
            <div>
              ...
              <div class="relative">
                <input
                  ...
                  max="100"
                  placeholder="10"
                  class="w-full rounded-xl border border-line px-3.5 py-2.5 pr-8 text-sm placeholder-gray-300 transition focus:border-brand/50 focus:outline-none focus:ring-2 focus:ring-brand/10"
                />
                <span class="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-sm font-bold text-gray-400">
                  %
                </span>
              </div>
            </div>
    ...
    <!-- Expires at -->
            <div>
              ...
              <input
                ...
                class="w-full rounded-xl border border-line px-3.5 py-2.5 text-sm text-gray-700 transition focus:border-brand/50 focus:outline-none focus:ring-2 focus:ring-brand/10"
              />
    ...
    <!-- Active toggle -->
            <div class="flex items-center gap-3 pt-1">
              ...
              />
              <label for="is_active_check" class="text-sm font-medium text-gray-700">
    ...
    <!-- Submit row spans full width -->
            <div class="flex items-center gap-3 sm:col-span-2 lg:col-span-3">
              <button
                type="submit"
                class="rounded-xl bg-brand px-6 py-2.5 text-sm font-semibold text-white transition hover:bg-[#a8001a]"
              >
              ...
              <button
                type="button"
                phx-click="cancel_form"
                class="rounded-xl border border-line px-5 py-2.5 text-sm font-medium text-gray-600 transition hover:bg-gray-50"
              >
                Cancel
              </button>
            </div>
        ...
        <%= if @promo_codes == [] do %>
          <div class="flex flex-col items-center py-16 text-center">
            ...
            <p class="mt-1 text-xs text-gray-400">
              Create your first code for an influencer campaign.
            </p>
          </div>
        <% else %>
            ...
            <table class="w-full">
              ...
              <tbody>
                <%= for promo <- @promo_codes do %>
                  <tr class="group border-b border-line transition-colors last:border-0 hover:bg-brand/3">
                    <!-- Code -->
    ...
    <!-- Status toggle -->
                    <td class="px-5 py-3.5">
                      <button
                        ...
                        title={
                          if promo.is_active, do: "Click to deactivate", else: "Click to activate"
                        }
                      >
                        <span class={[
                          "h-1.5 w-1.5 rounded-full",
                          if(promo.is_active, do: "bg-green-500", else: "bg-gray-400")
                        ]} />
                        ...
                        >
                          ...
                          >
                            <path
                              ...
                              stroke-linejoin="round"
                              stroke-width="2"
                              d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6M9 6V4a1 1 0 011-1h4a1 1 0 011 1v2M4 7h16"
                            />
                          </svg>
                        </button>
                      </div>
      ...
      <%= if @selected_code do %>
        <div class="overflow-hidden rounded-3xl border border-line bg-white shadow-sm">
          <div class="flex items-center justify-between border-b border-line px-6 py-5">
            <div>
              <h2 class="font-display uppercase tracking-wide text-base font-semibold text-ink">
                Orders using <span class="ml-1.5 font-mono text-brand">{@selected_code}</span>
              </h2>
            ...
            <div class="divide-y divide-line">
              <%= for order <- @selected_orders do %>
                <.link
                  ...
                  class="flex items-center gap-4 px-6 py-4 transition hover:bg-brand/3"
                >
