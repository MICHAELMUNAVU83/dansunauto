defmodule DansunautoWeb.AdminChatLive.Index do
  ...
  def handle_info({:new_message, %{session: session, message: _message}}, socket) do
    sessions =
      ...
      |> Enum.map(fn s ->
        if s.id == session.id,
          do: %{s | last_message_at: session.last_message_at, status: "active"},
          else: s
      end)
      |> then(fn list ->
  ...
  def render(assigns) do
    ...
    <div class="mb-8 flex items-center justify-between">
      ...
      <div class="flex items-center gap-2 rounded-xl border border-line bg-white p-1">
      ...
      <%= if Enum.empty?(visible) do %>
        <div class="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p class="mt-1.5 text-sm text-gray-400">
            Visitor conversations will appear here in real-time.
          </p>
        </div>
      <% else %>
