defmodule DansunautoWeb.AdminChatLive.Show do
  ...
  def handle_event("suggest_product", %{"id" => pid_str}, socket) do
    ...
    case Integer.parse(pid_str) do
      {pid, _} ->
        product = Dansunauto.Products.get_product!(pid)
  ...
  def render(assigns) do
      ...
      <div class="flex items-center gap-2">
        <%= if @session.status != "closed" do %>
          <button
            ...
            class="rounded-xl border border-line px-4 py-2 text-sm font-semibold text-gray-600 transition hover:border-gray-300 hover:text-ink"
          >
            Close Chat
          </button>
        <% else %>
    ...
    <%!-- Chat area --%>
    <div class="flex h-[calc(100vh-220px)] flex-col overflow-hidden rounded-2xl border border-line bg-white">
      <%!-- Messages --%>
      <div id="chat-messages" phx-hook="ChatScroll" class="flex-1 space-y-4 overflow-y-auto p-6">
                  ...
                  <div class="overflow-hidden rounded-2xl border border-line bg-white shadow-sm">
                    ...
                    <div class="p-3">
                      ...
                      <p class="mt-0.5 text-sm font-bold text-gray-700">
                        KES {DansunautoWeb.Format.price(msg.suggested_product_price)}
                      </p>
                      <a
      ...
      <%= if @show_product_search do %>
        <div class="border-t border-line bg-gray-50 p-4">
          ...
          <form phx-change="search_products" phx-submit="search_products">
            <input
              ...
              autofocus
              phx-debounce="200"
              class="w-full rounded-xl border border-line bg-white px-4 py-2.5 text-sm focus:border-gray-400 focus:outline-none"
            />
          </form>
          <%= if @product_results != [] do %>
            <div class="mt-3 space-y-2">
              <%= for p <- @product_results do %>
                <div class="flex items-center gap-3 rounded-xl border border-line bg-white p-3">
                  <%= if p.image do %>
                    <img
                      src={p.image}
                      class="h-12 w-12 flex-shrink-0 rounded-lg object-cover object-top"
                    />
                  <% end %>
                  <div class="min-w-0 flex-1">
                    <p class="truncate text-sm font-semibold text-ink">{p.name}</p>
                    <p class="text-xs text-gray-500">
                      KES {DansunautoWeb.Format.price(p.base_price)}
                    </p>
                  </div>
                  <button
      ...
      <div class="border-t border-line p-4">
        <.form for={%{}} phx-submit="send_reply" class="flex items-end gap-3">
          <div class="flex-1">
            <textarea
              name="reply"
              rows="2"
              ...
              class="w-full resize-none rounded-xl border border-line bg-gray-50 px-4 py-2.5 text-sm text-ink placeholder-gray-400 transition focus:border-gray-400 focus:bg-white focus:outline-none disabled:opacity-50"
            >{@reply_text}</textarea>
          </div>
            ...
            <button
              ...
              class={[
                ...
                if(@show_product_search,
                  ...
                  else: "border-line text-gray-400 hover:border-gray-400 hover:text-gray-700"
                )
              ]}
            >
