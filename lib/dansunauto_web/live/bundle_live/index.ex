defmodule DansunautoWeb.BundleLive.Index do
  use DansunautoWeb, :admin_live_view

  alias Dansunauto.Bundles
  alias Dansunauto.Bundles.Bundle
  ...
  @impl true
  def handle_info({DansunautoWeb.BundleLive.FormComponent, {:saved, bundle}}, socket) do
  ...
  def render(assigns) do
    ...
    <div class="relative mb-6 overflow-hidden rounded-3xl bg-gradient-to-r from-brand to-[#8b0014] px-7 py-6 text-white shadow-md">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <h1 class="mt-0.5 font-display uppercase tracking-wide text-2xl font-bold">Bundles</h1>
        </div>
                ...
                <.link navigate={~p"/admin/bundles/#{bundle}"} class="flex items-center gap-3">
                  <%= if bundle.image && bundle.image != "" do %>
                    <img
                      src={bundle.image}
                      alt={bundle.title}
                      class="h-10 w-10 flex-shrink-0 rounded-lg border border-line object-cover"
                    />
                  <% else %>
    ...
    >
      <.live_component
        module={DansunautoWeb.BundleLive.FormComponent}
        id={@bundle.id || :new}
        title={@page_title}
        action={@live_action}
        bundle={@bundle}
